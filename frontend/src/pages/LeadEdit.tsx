  ...
  const loadLead = async () => {
    try {
      ...
      setFormData({
        ...
        sanctioned_load_kva: lead.sanctioned_load_kva?.toString() || '',
        monthly_consumption_kwh: lead.monthly_consumption_kwh?.toString() || '',
        current_tariff_per_kwh: lead.current_tariff_per_kwh?.toString() || '',
        has_solar: lead.has_solar || false,
        solar_capacity_kw: lead.solar_capacity_kw?.toString() || '',
        has_battery: lead.has_battery || false,
        battery_capacity_kwh: lead.battery_capacity_kwh?.toString() || '',
        has_dg: lead.has_dg || false,
        dg_capacity_kva: lead.dg_capacity_kva?.toString() || '',
        roof_type: lead.roof_type || '',
        available_area_sqft: lead.available_area_sqft?.toString() || '',
        ownership_status: lead.ownership_status || '',
        lease_remaining_years: lead.lease_remaining_years?.toString() || '',
        lead_source: lead.lead_source || '',
        lead_source_detail: lead.lead_source_detail || '',
        deal_size_estimate: lead.deal_size_estimate?.toString() || '',
        preferred_model: lead.preferred_model || '',
        temperature: lead.temperature || LeadTemperature.COLD,
        lead_status: lead.lead_status || LeadStatus.NEW_LEAD,
      });
    } catch (err: any) {
          ...
          <Grid container spacing={3} sx={{ mb: 4 }}>
            <Grid item xs={12} md={4}>
              <TextField
                ...
                onChange={handleChange('lead_status')}
              >
